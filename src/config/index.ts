...
const config = {
    ...
    elasticsearch: {
        ...
        indexNames: {
            entities: env.get('INDEX_ENTITY').required().asString(),
            organizationGroups: env.get('INDEX_GROUPS').required().asString(),
            roles: env.get('INDEX_ROLES').required().asString(),
            digitalIdentities: env.get('INDEX_DIGITALIDENTITY').required().asString(),
        },
